...
define(['underscore'], function (_) {
    ...
    var Game = function (count_of_layers) {
        ...
        this._count_of_players = count_of_layers;
    ...
    Game.states = {
        ...
        victory_in_round: 2,
        dispute_in_round: 3,
        disputers_not_have_card_then_super_dispute_in_round: 4
    };
    ...
    Game.prototype.init = function () {
        ...
        var distribution_pack_of_cards_array = Game.distribute_pack_of_cards_array_for_players(
            shuffled_pack_of_cards_array, this._count_of_players);

        this._round_players_indexes_array = _.range(this._count_of_players);
    ...
    Game.prototype.player_is_playing_in_round = function (index) {
        return _.indexOf(this._round_players_indexes_array, index) != -1
    };
    ...
    Game.prototype.get_round_state_array = function () {
        ...
        if (round_winner_array.length === 1) {
            ...
            // if this.get_players_indexes_who_have_cards().length == 0 =>> may be Error? (all players not have cards)
            return [Game.states.disputers_not_have_card_then_super_dispute_in_round].concat(this.get_players_indexes_who_have_cards());
        }
    };
    ...
    Game.prototype._take_away_round_cards = function () {
        return this._take_away_cards_from_index_array(this._round_players_indexes_array);
    };

    Game.prototype.get_players_indexes_who_have_cards = function() {
        var indexes = [];
        for (var i = 0; i < this._players_cards_arrays.length; i++) {
            if (this._players_cards_arrays[i].length) {
                indexes.push(i);
    ...
    Game.prototype._update_next_round_play_playing_player_indexes = function (next_round_plying_player_indexes) {
        if (next_round_plying_player_indexes) {
            this._round_players_indexes_array = next_round_plying_player_indexes;
        } else {
            this._round_players_indexes_array = this.get_players_indexes_who_have_cards();
        }
    };
    ...
    Game.prototype.play_round = function () {
        ...
        if (state === Game.states.dispute_in_round) {
            ...
            if (round_cards.length === 0) {
                // may be only if state == Game.states.disputers_not_have_card_then_super_dispute_in_round
                throw new Error('Weary interesting bug #1');
            }
        ...
        } else if (state == Game.states.disputers_not_have_card_then_super_dispute_in_round) {
            if (round_cards.length != 0) {
                throw new Error('Weary interesting bug #2');
            }

            // super dispute around all players who can play
            var players_indexes_who_can_play_super_dispute_round = state_array.slice(1);

            // add dispute adds cards
            var added_dispute_cards = this._take_away_cards_from_index_array(players_indexes_who_can_play_super_dispute_round);
            this._dispute_cards_array.push.apply(this._dispute_cards_array, added_dispute_cards);

            this._update_next_round_play_playing_player_indexes(players_indexes_who_can_play_super_dispute_round);
        } else if (state == Game.states.game_over || state == Game.states.victory_in_round) {
    ...
    Game.prototype.get_round_playing_player_indexes_array = function () {
        return _.clone(this._round_players_indexes_array);
    };
